var predict_lien_tiep={
    storage_name:{
        key_du_doan :'lien_tiep_all_du_doan',
        curr_predict_continous :'lien_tiep_curr_predict_continous',
        curr_3_before_continous :'lien_tiep_curr_3_before_continous',
        curr_index_before_du_doan :'lien_tiep_curr_index_before_du_doan'
    },
    predict_next_step_series:function() {
        ...
        var obj_before_du_doan = localStorage.getItem(predict_lien_tiep.storage_name.curr_predict_continous);
        var arr_before_du_doan =[];
        if(obj_before_du_doan !=null){
            arr_before_du_doan = JSON.parse(obj_before_du_doan);
        }
        var curr_3_before_continous = localStorage.getItem(predict_lien_tiep.storage_name.curr_3_before_continous);
        
        var index_before_du_doan = localStorage.getItem(predict_lien_tiep.storage_name.curr_index_before_du_doan);
        ...
        if(arr_general_du_doan.length % 2 ==0){
            predict_lien_tiep.show_predict(arr_before_du_doan,index_before_du_doan,is_calc_predict);
            return;
        }
        ...
        if(curr_3_before_continous == 'ext-predict-t-t-t'){
            if(arr_du_doan[arr_du_doan.length-1] == false){
                predict_lien_tiep.show_predict([],0,is_calc_predict);
                predict_lien_tiep.set_current_data([],0,'ext-predict-t-t-t-f');
                return;
            }
        }
            ...
            if(arr_du_doan[arr_du_doan.length -1 ] == true){
                ...
                predict_lien_tiep.show_predict(arr_predict_special,1,is_calc_predict);
                predict_lien_tiep.set_current_data(arr_predict_special,1,'ext-predict-t-t-t-f-t');    
            }
            else{
                var arr_predict_special = [false,false,false,true,true,false,false,true];
                predict_lien_tiep.show_predict(arr_predict_special,1,is_calc_predict);
                predict_lien_tiep.set_current_data(arr_predict_special,1,'ext-predict-t-t-t-f-f');    
            }
            return;
        }
            ...
            if(arr_du_doan[arr_du_doan.length-1] == true){
                predict_lien_tiep.show_predict([],0,is_calc_predict);
                predict_lien_tiep.set_current_data([],0,'ext-predict-f-f-f-t');
                return;
            }
        }
            ...
            if(arr_du_doan[arr_du_doan.length-1] == true){
                ...
                predict_lien_tiep.show_predict(arr_predict_special,1,is_calc_predict);
                predict_lien_tiep.set_current_data(arr_predict_special,1,'ext-predict-f-f-f-t-t');
            }
            else{
                var arr_predict_special =[false,false,true,false,true,false,true,false];
                predict_lien_tiep.show_predict(arr_predict_special,1,is_calc_predict);
                predict_lien_tiep.set_current_data(arr_predict_special,1,'ext-predict-f-f-f-t-f');
            }
            return;
        }
        else{
        ...
        if(is_repeat_du_doan){
            ...
            predict_lien_tiep.show_predict(arr_predict,0,is_calc_predict);
            predict_lien_tiep.set_current_data(arr_predict,0,key);

        }
        else{
            index_before_du_doan += 1;
            predict_lien_tiep.show_predict(arr_before_du_doan,index_before_du_doan,is_calc_predict);
            predict_lien_tiep.set_current_data_without_curr_3_before_continous(arr_before_du_doan,index_before_du_doan);
        }
    },  
    ...
    set_current_data : function(arr_curr_predict,curr_index,curr_3_before){
        ...
        localStorage.setItem(predict_lien_tiep.storage_name.curr_index_before_du_doan,curr_index);
        localStorage.setItem(predict_lien_tiep.storage_name.curr_3_before_continous,curr_3_before);
    },
