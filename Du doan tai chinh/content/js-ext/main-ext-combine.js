...
$(document).ready(function(){
    ...
    $('#btnRollback').off('click').on('click',function(){
        var obj= localStorage.getItem(global_key_combine.key_du_doan);
        var arr_du_doan =[];
        if(obj != null){
...
function btnExact_Click(){
    ...
    predict_chan_combine.predict_next_step_series();
...
function append_local_storage_trung_or_truot(is_trung){
    var obj= localStorage.getItem('all_du_doan_combine_4k_9k');
    var arr_du_doan =[];
    if(obj != null){
        arr_du_doan = JSON.parse(obj);   
    }    
    arr_du_doan.push(is_trung);
    localStorage.setItem('all_du_doan_combine_4k_9k',JSON.stringify(arr_du_doan) );    
}
...
function append_local_storage_trung_or_truot_remain() {
    ...
    if (lgt_du_doan > 0) {
        if (lgt_du_doan % 2 == 0) {
            push_item_into_array_local_storage(global_key_combine.key_du_doan_chan, arr_du_doan[lgt_du_doan - 1]);
        }
        else {
            push_item_into_array_local_storage(global_key_combine.key_du_doan_le, arr_du_doan[lgt_du_doan - 1]);
        }
    }
    if (lgt_du_doan >= 2) {
        if (lgt_du_doan % 2 == 0) {
            if (arr_du_doan[lgt_du_doan - 1] == arr_du_doan[lgt_du_doan - 2]) {
                push_item_into_array_local_storage(global_key_combine.key_du_doan_chan_le_tach_biet, true);
            }
            else {
                push_item_into_array_local_storage(global_key_combine.key_du_doan_chan_le_tach_biet, false);
            }
        }
        else{
            if (arr_du_doan[lgt_du_doan - 1] == arr_du_doan[lgt_du_doan - 2]) {
                push_item_into_array_local_storage(global_key_combine.key_du_doan_chan_le_lien_tiep, true);
            }
            else {
                push_item_into_array_local_storage(global_key_combine.key_du_doan_chan_le_lien_tiep, false);
            }
        }
    }
}
...
function predict_from_continue_result(){
    var obj_du_doan = localStorage.getItem(global_key_combine.key_du_doan);
    var arr_du_doan =[];
    if(obj_du_doan != null){
    ...
    var curr_3_before_continous = localStorage.getItem(global_key_combine.curr_3_before_continous) ;
    // Lấy ra kết quả đã dự đoán trước đó
    var obj_before_du_doan = localStorage.getItem(global_key_combine.curr_predict_continous);
    var index_before_du_doan = localStorage.getItem(global_key_combine.curr_index_continous);
    ...
    if(curr_3_before_continous =='predict-t-f-f'){
        if(arr_du_doan[arr_du_doan.length -1] == true){
            ...
            localStorage.setItem(global_key_combine.curr_predict_continous,JSON.stringify([]) );
            localStorage.setItem(global_key_combine.curr_index_continous,0);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-t-f-f-t');
            return;
        }
    }
        ...
        if (arr_du_doan[arr_du_doan.length - 1] == true) {
            ...
            localStorage.setItem(global_key_combine.curr_predict_continous, JSON.stringify(arr_predict_special) );
            localStorage.setItem(global_key_combine.curr_index_continous,1);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-t-f-f-t-t');
        }
        else {
            ...
            localStorage.setItem(global_key_combine.curr_predict_continous, JSON.stringify(arr_predict_special) );
            localStorage.setItem(global_key_combine.curr_index_continous,1);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-t-f-f-t-f');
        }
        return;
    }
        ...
        if(arr_du_doan[arr_du_doan.length -1] == true){
            ...
            localStorage.setItem(global_key_combine.curr_predict_continous,JSON.stringify([]) );
            localStorage.setItem(global_key_combine.curr_index_continous,0);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-f-f-f-t');
            return;
        }
    }
        ...
        if (arr_du_doan[arr_du_doan.length - 1] == true) {
            ...
            localStorage.setItem(global_key_combine.curr_predict_continous, JSON.stringify(arr_predict_special));
            localStorage.setItem(global_key_combine.curr_index_continous,1);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-f-f-f-t-t');
        }
        else {
            var arr_predict_special = [false, false, true, false, true, false, true, false];
            show_predict_result_in_array(arr_predict_special, 1);
            localStorage.setItem(global_key_combine.curr_predict_continous, JSON.stringify(arr_predict_special) );
            localStorage.setItem(global_key_combine.curr_index_continous,1);
            localStorage.setItem(global_key_combine.curr_3_before_continous,'predict-f-f-f-t-f');
        }
        return;
    }
    else{
    ...
    if(is_repeat_du_doan){
        ...
        localStorage.setItem(global_key_combine.curr_predict_continous,JSON.stringify(arr_predict) );
        localStorage.setItem(global_key_combine.curr_index_continous, 0);
        localStorage.setItem(global_key_combine.curr_3_before_continous, key);
        ...
        localStorage.setItem(global_key_combine.curr_predict_continous,JSON.stringify(arr_before_du_doan) );
        localStorage.setItem(global_key_combine.curr_index_continous,index_before_du_doan);
    }
}
...
function show_predict_result_in_array(arr_predict,suggest_chose){
    ...
    $('label[name=money_predict_continuous]').html('Số tiền : ' + type_money + global_key_combine.money[suggest_chose]);
    $('label[name=money_predict_continuous]').data('typemoney', type_money);
    $('label[name=money_predict_continuous]').data('amountmoney', global_key_combine.money[suggest_chose]);
    //general_lib.calc_total(type_money,global_key_combine.money[suggest_chose]);
}

var calc_combine_4k_9k ={
    key_storage:{
        frame_1st :{
            result :'three_frame_combine_1st_2st_3st_du_doan_next'
        },
        frame_2st :{
            result :'three_frame_combine_2st_3st_4st_du_doan_next'
        },
        frame_3st :{
            result :'three_frame_combine_3st_4st_5st_du_doan_next'
        },
        frame_4st :{
            result :'three_frame_combine_4st_5st_6st_du_doan_next'
        },
        frame_5st :{
            result :'three_frame_combine_5st_6st_7st_du_doan_next'
        },
        frame_6st :{
            result :'three_frame_combine_6st_7st_8st_du_doan_next'
        },
        frame_7st :{
            result :'three_frame_combine_7st_8st_9st_du_doan_next'
        },
        frame_8st :{
            result :'three_frame_combine_8st_9st_1st_du_doan_next'
        },
        frame_9st :{
            result : 'three_frame_combine_9st_1st_2st_du_doan_next'
        }
    },
    calc :function(frm){
        var arr_result = general_lib.get_array_from_local_storage(frm.result);
